...
description: 从一次 401 无效的令牌说起，讲清楚 OpenAI 兼容端点要打通要过几关（DNS/TCP/TLS、/models 认证、/chat/completions 推理、openai 0.28 与 1.x 兼容），并给出零依赖 Python 与纯 curl 两个实现：退出码直接给结论，密钥用 openssl 加密后存进 env 配置文件，附 mock 服务跑出的 27 项分支测试。
date: 2026-09-25
...
这篇文章对应两个文件：`llm_probe.py`（869 行，纯标准库）和 `llm_probe.sh`（773 行，纯 `curl` + `openssl`）。它们做同一件事——**把"连不上"翻译成"哪一层连不上"**，用退出码直接给结论；并且把密钥加密存进一份 env 配置文件，两个实现互认对方写的密文。

完整代码在 GitHub：<https://github.com/yongchaowu/llm-probe>，本地位于 `~/Workspace/VibeCoding/llm-probe/`。仓库里还有一份"最简 demo"（`demo_minimal.py`，85 行），见第六节；本文附录收录了两份脚本的逐行源码。
...
**结论要可信，得先在自己能控制的环境里把所有分支跑一遍。** 于是写了个 ~120 行的 mock 服务（`MOCK_MODE` 切换行为），覆盖 10 组场景、27 项断言：
...
口令**永远不写进配置文件**（写了等于没加密），来源按优先级：

1. 环境变量 `LLM_PASSPHRASE`
2. `LLM_PASSPHRASE_FILE` 指向的口令文件（建议 `chmod 600`）
3. 终端交互输入（`getpass` / `stty -echo`，不回显、不进 shell 历史）
...
分层探测是排障工具，不是入门材料。如果你只是想**先跑通一次调用**、看看端点到底能不能用，那 869 行的 `llm_probe.py` 显得太重了——所以仓库里另放了一个 `demo_minimal.py`，85 行、只有三个函数，只做一件事：发一条消息，打印回复。
...
一致性靠测试钉住：同一组 mock 场景跑两遍、比对退出码，密文互写互读各测一次。**"应该是一样的"不算数，跑过才算。**
...
- **测试要造自己的对照组**：mock 服务把 10 组分支跑干净，才敢说"这 27 项都对"；也才有机会抓到子 shell 丢赋值、`\uXXXX` 转义、argparse 覆盖这三个不跑就发现不了的 bug。

---

## 附录：完整源码

### A. `llm_probe.py`（869 行，Python 3.7+，零第三方依赖）
...
### B. `llm_probe.sh`（773 行，POSIX sh，依赖 curl + openssl）
...
### C. 配置文件模板 `llm_probe.env.example`（32 行）
...
### D. `demo_minimal.py`（85 行，最简 demo）
...
> 测试用的 mock 服务（`tests/mock_llm.py`，6 种模式：`ok / unauthorized / badpath / badmodel / ratelimit / noreduce`）和分支测试脚本（`tests/run_tests.sh`，27 项断言）随代码一起放在 `~/Workspace/VibeCoding/llm-probe/tests/`，直接 `./tests/run_tests.sh` 即可复现第四节的全部结论（期望 `PASS=27 FAIL=0`）。测试刻意不读取真实配置的解密口令：前 8 组用 `--key` 显式传密钥，密文互通那组用临时配置配自己的口令。
